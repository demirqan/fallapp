...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onResume() {
        ...
        currentUser = dbHelper.getUser(); // Veritabanından güncel kullanıcıyı al
    ...
    private void syncPremiumStatus() {
        ...
        boolean isPremiumDb = currentUser.isPremium();

        SharedPreferences prefs = getSharedPreferences("app_prefs", MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putBoolean("is_premium", isPremiumDb);
        editor.apply();
    }
    ...
    private void openFortuneReading(String readingType) {
        ...
        // Eğer premium değilse ve yeterli altın yoksa uyarı göster
        if (!isPremium && currentUser.getCoins() < 10) {
            Toast.makeText(this, "Yeterli altınınız yok. Lütfen altın satın alın.", Toast.LENGTH_SHORT).show();
            startActivity(new Intent(MainActivity.this, CoinPurchaseActivity.class));
            return;
        }

        // Falcı listesi ekranına geç
